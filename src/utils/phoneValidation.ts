...
export function validatePhoneNumber(phone: string): { isValid: boolean; error?: string } {
  ...
  if (cleaned.startsWith('+')) {
    ...
    isValid = /^\+[1-9]\d{6,14}$/.test(cleaned) && digitCount >= 7 && digitCount <= 15;
  } else {
  ...
  if (!isValid) {
    if (cleaned.startsWith('+') && digitCount < 7) {
      return { isValid: false, error: 'Phone number with country code must have at least 7 digits' };
      ...
      return { isValid: false, error: 'Country code must start with a digit 1-9' };
    } else {

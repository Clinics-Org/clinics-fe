 ...
 */
export function extractValidationErrors(error: ApiError | Error | any): Record<string, string> {
  ...
  if (error?.details && typeof error.details === 'object') {
    Object.entries(error.details).forEach(([field, messages]) => {
      ...
      if (field.includes('.')) {
        ...
        const prescriptionItemsIndex = parts.findIndex(part => part === 'prescription_items');
        if (prescriptionItemsIndex !== -1 && parts.length > prescriptionItemsIndex + 1) {
          // Extract index and field name
          const index = parts[prescriptionItemsIndex + 1];
          const fieldName = parts[prescriptionItemsIndex + 2] || parts[parts.length - 1];
...
export function hasValidationErrors(error: ApiError | Error | any): boolean {
  return !!(error?.details && typeof error.details === 'object' && Object.keys(error.details).length > 0);
}
